...
const SC_offered_to_MD = () => {
  ...
  const [selectedOfferedBy, setSelectedOfferedBy] = useState('');
  ...
  const handleOfferedByChange = (event) => {
    setSelectedOfferedBy(event.target.value);
  };
  ...
  const handleSubmit = () => {
    if (selectedDepartment.length < 2) {
      alert('Please select at least 2 departments.');
    }
  ...
  const sendDataToPhp = (selectedProgram, selectedCourseType, courseName, selectedSlot, selectedLecture, selectedTutorial, selectedPractical, selectedTotalMods, selectedWeeklyHours, selectedSemester, selectedDepartment) => {
    ...
    axios.post('http://localhost/test/SCSD/test_6.php', data)
      .then(response => {        
  ...
  return (
    ...
    <center><h2>Single-course Offered to Multiple Departments</h2></center>
      ...
      <div className="form-field">
        ...
        {['IT', 'ME', 'AE', 'CE', 'CH', 'CS', 'EC', 'EE', 'EN', 'ES', 'MA', 'PH', 'PE', 'AD'].map(department => (
          <label key={department}>
            <input
              type="radio"
              id={`offeredBy${department}`}
              name="offeredBy"
              value={department}
              checked={selectedOfferedBy === department}
              onChange={handleOfferedByChange}
            />
            <label htmlFor={`offeredBy${department}`}>{department}</label>
          </label>
        ))}
      </div>
  
      <div className="form-field">
        <label>Choose Departments getting offered:</label>
        {['IT', 'ME', 'AE', 'CE', 'CH', 'CS', 'EC', 'EE', 'EN', 'ES', 'MA', 'PH', 'PE', 'AD'].map(department => (
          <div key={department}>
            <input
              type="checkbox"
...
export default SC_offered_to_MD;
