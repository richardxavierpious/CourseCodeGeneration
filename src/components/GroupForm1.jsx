...
const GroupForm = () => {
  const [formStates, setFormStates] = useState([
    { tableName: 'degree', values: ['', '', ''], isRequired: true, btnNumber: 1, columns: ["Degree Code", "Degree Exp", "Degree short"], proper_name : "Update Previously Added Degree" },
    { tableName: 'dept', values: ['', '', ''], isRequired: true, btnNumber: 2, columns: ["Department ID", "Department Name", "Department Short"], proper_name : " Update Previously Added Department" },
    { tableName: 'programme', values: ['', '', '', '', '', ''], isRequired: true, btnNumber: 3, columns: ["Programme ID", "Programme Name", "Programme short", "Department ID", "Code", "Degree ID"], proper_name : "Update Previously Added Program" },
    { tableName: 'coursetype', values: ['', '', '', '', '', '', ''], isRequired: true, btnNumber: 4, columns: ["CrsTypeId", "Course Type ID", "Course Type","Work Load","Effective Date","Description","Course Type Name"], proper_name : "Update Previously Added Course Type" },
  ]);
  ...
  const handleFormSubmit = (e) => {
    ...
    axios.post('http://localhost/test/Revision/update_into_table.php', formData)
      .then(response => {
